...
def mostrar_formula():
    ...
    frame_datos.config(bg="LightPink")
    
    if figura == "Triángulo":
        tk.Label(frame_datos, text="Fórmula: Área = (base × altura) / 2").pack()
        tk.Label(frame_datos, text="Base:" ,bg="LightPink").pack()
        entradas["base"] = tk.Entry(frame_datos)
        entradas["base"].pack()
        tk.Label(frame_datos, text="Altura:" ,bg="LightPink").pack()
        ...
        tk.Label(frame_datos, text="Lado:" , bg="LightPink").pack()
        entradas["lado"] = tk.Entry(frame_datos)
        entradas["lado"].pack()
    elif figura == "Círculo":
        tk.Label(frame_datos, text="Fórmula: Área = 3.1416 × radio²").pack()
        tk.Label(frame_datos, text="Radio:" ,bg="LightPink").pack()
        entradas["radio"] = tk.Entry(frame_datos)
        entradas["radio"].pack()

    tk.Button(frame_datos, text="Calcular área", command=calcular_area ,bg="LightPink").pack(pady=10)
